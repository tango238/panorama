...
3. `$VAULT/Dashboard.md` を読み、現在のセッションID と一致する `<!-- session: {id} -->` を持つカードを 🟢/🟡 から検索。見つかれば `/panorama done` のステップ2〜6を全て実行して自動完了する（アーカイブ＋✅完了へ移動）。セッションIDで見つからない場合は、同じ `path` のカードをフォールバック検索する。
...
**重要: 全ステップを順番通りに実行すること。ステップのスキップ禁止。**
